...
y49vl_loaded=np.load('/home/claude/y49vl.npy')
assert np.allclose(y49vl, y49vl_loaded), "Val mismatch!"
...
cp=dict(iterations=5000,learning_rate=0.02,depth=8,loss_function='RMSE',random_seed=42,verbose=0,l2_leaf_reg=3,min_data_in_leaf=5)
mc=CatBoostRegressor(**cp)
mc.fit(Xc,yc,sample_weight=wc,eval_set=(X49vl,y49vl),early_stopping_rounds=300)
vc=mc.predict(X49vl)
print(f"CAT val: {max(0,100*r2_score(y49vl,vc)):.4f} iter={mc.best_iteration_}")
mcf=CatBoostRegressor(**{**cp,'iterations':mc.best_iteration_+100,'verbose':0})
mcf.fit(Xa,ya,sample_weight=wa)
pc=mcf.predict(Xt)
np.save('/home/claude/pred_cat_v5.npy',pc); np.save('/home/claude/val_cat_v5.npy',vc)
print(f"CAT done, pred mean={pc.mean():.4f}")

print("Training XGB...")
xp={'objective':'reg:squarederror','max_depth':8,'learning_rate':0.02,'subsample':0.8,'colsample_bytree':0.7,'tree_method':'hist','seed':42,'verbosity':0}
dtr=xgb.DMatrix(Xc,label=yc,weight=wc); dvl=xgb.DMatrix(X49vl,label=y49vl); dts=xgb.DMatrix(Xt)
mx=xgb.train(xp,dtr,5000,[(dvl,'val')],early_stopping_rounds=300,verbose_eval=False)
vx=mx.predict(dvl)
...
print(f"XGB done, pred mean={px.mean():.4f}")